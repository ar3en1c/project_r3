...
def top250_view(request):
    def ranked(qs, rows):
        ids = [imdb_id for imdb_id, _ in rows]
        names = dict(rows)
        found = {
            r.remote_id: obj
        ...
        return [
            {"rank": i, "item": found[imdb_id], "name": names[imdb_id]}
            for i, imdb_id in enumerate(ids, 1)
            if imdb_id in found
        ]

    movies = ranked(Movies.objects.all(), _read_rows(MOVIES_CSV))
    series = ranked(Series.objects.all(), _read_rows(SERIES_CSV))
