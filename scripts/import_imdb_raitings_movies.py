...
from movies.models import Movies, RemoteId
...
def import_ratings(
    ...
    print(
        f"{action} {totals['updated']} movies from {totals['rows']} IMDb rows "
...
def update_batch(ratings, source_name, batch_size, dry_run, totals):
    ...
    for remote_id in remote_ids.iterator(chunk_size=batch_size):
        ...
        if remote_id.movies.rate != new_rating:
            remote_id.movies.rate = new_rating
            movies_to_update.append(remote_id.movies)

    totals["updated"] += len(movies_to_update)

    if not dry_run and movies_to_update:
        with transaction.atomic():
            Movies.objects.bulk_update(
                movies_to_update,
                ["rate"],
